...
def _templates() -> list[str]:
    """Return sorted list of PNG filenames in _cfg.WINDOW_LIST_DIR."""
    if not os.path.isdir(_cfg.WINDOW_LIST_DIR):
        return []
    return sorted(
        f for f in os.listdir(_cfg.WINDOW_LIST_DIR) if f.lower().endswith(".png")
    )
...
def locate_window() -> tuple[int, int] | None:
    ...
    for fname in _templates():
        result = match_template(fname, _cfg.WINDOW_LIST_DIR,
                                threshold=_cfg.WINDOW_MATCH_THRESHOLD)
        if result is not None:
            # match_template returns center; convert to top-left
            return result[0] - _cfg.GAME_WINDOW_W // 2, result[1] - _cfg.GAME_WINDOW_H // 2
    return None
...
def quick_check_window(expected_x: int, expected_y: int) -> bool:
    ...
    for fname in _templates():
        ...
        if result is not None:
            cx, cy = result
            wx = cx - _cfg.GAME_WINDOW_W // 2
            wy = cy - _cfg.GAME_WINDOW_H // 2
